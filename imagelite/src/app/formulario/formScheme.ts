...
export const formValidationScheme = Yup.object().shape({
    ...
    file: Yup.mixed<Blob>()
    .required('Select an image to uploud!')
    .test('size', 'File size cannout be higher than 4 MB', (file) => {
        return file.size < 4000000;
    })
    .test('type', 'Accepted formats: jpeg, giff or png', (file) => {
        return file.type === 'image/jpeg' || file.type === 'image/png' || file.type === 'image/gih';
    })
})